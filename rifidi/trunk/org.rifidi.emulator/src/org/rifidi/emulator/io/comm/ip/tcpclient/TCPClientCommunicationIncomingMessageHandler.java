 ...
 * and stores the them as they arrive. Runs until the client socket is closed.
 * 
 ...
 * 
 */
public class TCPClientCommunicationIncomingMessageHandler implements Runnable {

	/**
	 * The logger for this class.
	 */
	private static Log logger = LogFactory
			.getLog(TCPClientCommunicationIncomingMessageHandler.class);
	...
	public void run() {
		...
		Constructor c = null;
		try {
			/* The class has to have a single parameter of type InputStream */
			...
			while (keepRunning) {
				...
				if (socketIn != null) {
					...
					try {
						...
						logger.debug("The Incoming message is:"
								+ ByteAndHexConvertingUtility
										.toHexString(buffer));
					} catch (IOException e) {
						/* Drop the socket */
						buffer = null;
