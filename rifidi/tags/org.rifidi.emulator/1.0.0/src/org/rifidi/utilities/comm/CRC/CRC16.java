package org.rifidi.utilities.comm.CRC;

public class CRC16 {

	/**
	 * This method cacluates the crc16 for a array of bytes using the CCITT
	 * polynomial
	 * 
	 * @param bytes
	 ...
	 *            The seed value
	 * @param table
	 *            The table of values to use
	 * @param invert
	 *            If true, will preform a bitwise inversion on the crc value
	 *            before returning
	 * @return The CRC value
	 */
	public static int calculateCRC(byte[] bytes, int seedValue, int[] table,
			boolean invert) {

		for (byte b : bytes) {
			seedValue = (seedValue >>> 8) ^ table[(seedValue ^ b) & 0xff];
		}
		if (invert) {
			return seedValue ^ 0xFFFF;
		} else
			return seedValue;
	}

	/**
	 * This is the CRC Table that the Symbol Reader uses with a seed value of
	 * 0xBEEF
	 */
	public static int[] XR400_CRC_TABLE = { 0x0000, 0x1189, 0x2312, 0x329b,
