 ...
 */
public abstract class SimpleThreadedJmeContext {
	private PhysicsSpace physicsSpace = PhysicsSpace.create();

	/**
	 * The logger for this class
	 */
	private static Log logger = LogFactory.getLog(SimpleThreadedJmeContext.class);
	 ...
	 */
	private UpdatingThread updateThread;
	 ...
	 */
	public SimpleThreadedJmeContext( GLCanvas canvas ) {
		...
		renderThread = new RenderingThread(lock,renderQueue,rootNode,canvas, Display.getCurrent());
		updateThread = new UpdatingThread(lock,updateQueue,rootNode);
...
//		GameTaskQueue render = GameTaskQueueManager.getManager().getQueue(GameTaskQueue.RENDER);
	 ...
	 */
	public RenderingThread getRenderThread() {
		return renderThread;
	}
	 ...
	 */
	public UpdatingThread getUpdateThread() {
		return updateThread;
	}
